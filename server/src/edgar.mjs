...
const METRICS = [
  ...
  { key: 'operatingIncome', unit: 'USD', round: roundWhole, tags: ['OperatingIncomeLoss'] },
  { key: 'epsDiluted', unit: 'USD/shares', round: round2, tags: ['EarningsPerShareDiluted'] },
  {
    key: 'sharesDiluted',
    unit: 'shares',
    round: roundWhole,
  ...
  { key: 'totalDebt', unit: 'USD', round: roundWhole, tags: ['LongTermDebtNoncurrent', 'LongTermDebt'] },
]
...
const BALANCE_SHEET_METRICS = [
  ...
  { key: 'longTermDebt', unit: 'USD', round: roundWhole, tags: ['LongTermDebtNoncurrent', 'LongTermDebt'] },
...
const ADDITIVE_QUARTER_METRIC_KEYS = new Set(['revenue', 'netIncome', 'grossProfit', 'operatingIncome'])
...
function buildQuarters(companyFacts, periods) {
  ...
  const directPeriodEnds = new Set(
    [...directPeriodKeys].map((key) => key.split('|')[1]).filter((end) => typeof end === 'string' && end.length > 0),
  )
...
function buildFiscalYears(companyFacts) {
  ...
  const relevantYears = new Set([...metricMaps.get('revenue').keys(), ...metricMaps.get('netIncome').keys()])
...
export function deriveFundamentals(companyFacts) {
  const periods = annualPeriods(companyFacts)
  return {
    fiscalYears: buildFiscalYears(companyFacts),
    quarters: buildQuarters(companyFacts, periods),
  }
}
...
async function tickerCikMap() {
  if (!tickerCikMapPromise) {
    tickerCikMapPromise = rateLimitedFetchJson(SEC_TICKER_MAP_URL, 'SEC ticker map')
      .then(buildTickerCikMap)
      .catch((error) => {
...
function resolveCik(ticker, tickerMap) {
  for (const candidate of secTickerCandidates(ticker)) {
    const result = tickerMap.get(candidate)
...
export async function fetchFundamentals(ticker) {
  ...
  const cikInfo = resolveCik(normalized, await tickerCikMap())
  if (!cikInfo) return null

  const factsUrl = `${SEC_COMPANY_FACTS_URL}/CIK${cikInfo.cik}.json`
  const companyFacts = await rateLimitedFetchJson(factsUrl, `${normalized} companyfacts`)
  if (!companyFacts) return null

  const { fiscalYears, quarters } = deriveFundamentals(companyFacts)
